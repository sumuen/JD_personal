...
if properties.get("proxy"):
    ...
    client = TelegramClient("magic", api_id, api_hash, proxy=proxy, auto_reconnect=True, retry_delay=1,
                            connection_retries=99999).start()
else:
    client = TelegramClient("magic", api_id, api_hash, auto_reconnect=True, retry_delay=1,
                            connection_retries=99999).start()
        ...
        try:
            ...
            if curr_queue.qsize() > 1:
                await client.send_message(bot_id,
                                          f'{action["name"]}，队列长度{curr_queue.qsize()}，将等待{action["wait"]}秒...')
